...
export function addFontFamilySetting(containerEl: HTMLElement, plugin: SourceModeStyling) {
	...
	fontModeSelect.value = plugin.settings.fontFamily && plugin.settings.fontFamily !== 'theme' ? plugin.settings.fontFamily : 'theme';
	fontSetting.controlEl.appendChild(fontModeSelect);
	fontModeSelect.addEventListener('change', async () => {
		plugin.settings.fontFamily = fontModeSelect.value;
