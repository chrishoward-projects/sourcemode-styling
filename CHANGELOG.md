...
### Added
- A one-time notice for users on Obsidian below 1.13.0, telling them this is the last version they will be offered and that later releases need a newer Obsidian. It stays on screen until dismissed and is never shown again.
...
### Fixed
- Source mode styling is no longer overridden by the Minimal theme, which tied this plugin's editor rule on specificity and won on load order. The configured font, size, line height, colours and weight now apply under Minimal.
...
### Fixed
- Disabling the plugin now clears source mode styling from editors in popout windows, which the previous document-scoped lookup could not reach
- A pending view mode update can no longer re-apply styling after the plugin is disabled

### Changed
- Collapsed three separate timers into one debounced view mode update, so opening a file no longer triggers the same work three times
...
- Fix settings not using sentence case
- Fix styles being set directly, not by CSS
...
### Changed
- Removed Enable Source Mode Styling setting references from README documentation
- Updated Quick Start instructions to reflect setting removal
- Removed setting from Configuration Options table
...
## Fixed:
- Removed inline styles from font detection to comply with Obsidian submission guidelines
- Font detection now uses CSS injection via style elements instead of setProperty()

## [0.2.16] - 2025-08-17

## Fixed:
- Error in mainfest version
